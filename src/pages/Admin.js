...
export function ActivityLogTab({ logs, total, page, loading, filter, setFilter, onPageChange, onRefresh }) {
  ...
  const ACTION_OPTIONS = [
    ...
    ["appointment_booked", "Appointment Booked"], ["appointment_status_changed", "Appointment Updated"],
    ["queue_next", "Queue Next"], ["queue_reset", "Queue Reset"],
    ["medicine_added", "Medicine Added"], ["medicine_updated", "Medicine Updated"],
    ["medicine_deleted", "Medicine Deleted"], ["medicine_stock_updated", "Stock Updated"],
    ["user_created", "User Created"], ["user_updated", "User Updated"],
    ["user_deleted", "User Deleted"], ["user_disabled", "User Disabled"],
    ["user_enabled", "User Enabled"], ["notice_published", "Notice Published"],
  ];
    ...
    <div style={{ animation: "slideIn 0.3s ease" }}>
      ...
      {PAGES > 1 && (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 4px" }}>
          <span style={{ fontSize: "13px", color: "#9ca3af" }}>Page {page} of {PAGES} · {total.toLocaleString()} total records</span>
...
export default function Admin() {
  ...
  // Medicine state — ✅ added supplier, expiryDate, entryDate
  const [medForm, setMedForm] = useState({
    name: "", desc: "", price: "", category: "",
    img: "", stock: "100", lowStockThreshold: "10", unit: "units",
    supplier: "", expiryDate: "", entryDate: new Date().toISOString().split("T")[0],
  });
  ...
  useEffect(() => {
    try {
      ...
      if (isLoggedIn !== "true" || role !== "admin") { navigate("/login", { replace: true }); return; }
      setAuthChecked(true);
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        ...
        if (aptsR.status === "fulfilled" && aptsR.value.ok)   { newApts  = sanitizeObjectArray(await aptsR.value.json());  setAppointments(newApts); }
        if (usersR.status === "fulfilled" && usersR.value.ok) { newUsers = sanitizeObjectArray(await usersR.value.json()); setUsers(newUsers); }
        if (ordersR.status === "fulfilled" && ordersR.value.ok) {
          ...
          prevOrdersRef.current = fetched; newOrders = fetched; setOrders(fetched);
        }
        if (medsR.status === "fulfilled" && medsR.value.ok) { newMeds = sanitizeObjectArray(await medsR.value.json()); setMedicines(newMeds); }
        if (lowR.status === "fulfilled" && lowR.value.ok)   { newLow  = sanitizeObjectArray(await lowR.value.json());  setLowStockMeds(newLow); }
        try {
          localStorage.setItem("admin_cache", JSON.stringify({ appointments: newApts||[], users: newUsers||[], orders: newOrders||[], medicines: newMeds||[], lowStock: newLow||[], ts: Date.now() }));
        } catch { /* storage full */ }
  ...
  useEffect(() => { if (activeTab !== "analytics" || !authChecked) return; fetchAnalytics(); }, [activeTab, authChecked]); // eslint-disable-line
  ...
  const callNextPatient = async (type) => {
    ...
    } catch { setNotification({ open: true, message: "Failed to advance queue", severity: "error" }); }
    finally { setQueueLoading((prev) => ({ ...prev, [type]: false })); }
  };
  const resetQueue = async (type) => {
    ...
    } catch { setNotification({ open: true, message: "Failed to reset queue", severity: "error" }); }
  };
  ...
  useEffect(() => {
    ...
    import("socket.io-client").then(({ io }) => {
      ...
      socket.on("queue:update", (data) => setQueueStatus((prev) => ({ ...prev, [data.type]: data })));
      fetchQueueStatus("appointment"); fetchQueueStatus("order"); fetchQueueStatus("walkin");
      return () => socket.disconnect();
    }).catch(() => { fetchQueueStatus("appointment"); fetchQueueStatus("order"); fetchQueueStatus("walkin"); });
  }, [authChecked]); // eslint-disable-line
  ...
  const filteredUsers  = safeUsers.filter((u) => { const q = userSearch.toLowerCase().trim(); if (!q) return true; return String(u?.name||"").toLowerCase().includes(q)||String(u?.email||"").toLowerCase().includes(q)||String(u?.phone||"").toLowerCase().includes(q); });
  const filteredOrders = safeOrders.filter((o) => { const q = orderSearch.toLowerCase().trim(); if (!q) return true; return String(o?.userId?.name||"").toLowerCase().includes(q)||String(o?.userId?.email||"").toLowerCase().includes(q)||String(o?.status||"").toLowerCase().includes(q)||String(o?._id||"").toLowerCase().includes(q); });
  const filteredApts   = safeAppointments.filter((a) => { const q = aptSearch.toLowerCase().trim(); if (!q) return true; return String(a?.name||"").toLowerCase().includes(q)||String(a?.contact||"").toLowerCase().includes(q)||String(a?.problem||"").toLowerCase().includes(q)||String(a?.status||"").toLowerCase().includes(q); });

  const statusColors = { Delivered: { bg: "#dcfce7", color: "#166534" }, Approved: { bg: "#dbeafe", color: "#1e40af" }, "Out for Delivery": { bg: "#fef9c3", color: "#854d0e" }, Cancelled: { bg: "#fee2e2", color: "#991b1b" }, Pending: { bg: "#fef3c7", color: "#92400e" }, Completed: { bg: "#dcfce7", color: "#166534" } };
  const aptStatusColors = { Pending: { bg: "#fef3c7", color: "#92400e" }, Confirmed: { bg: "#dbeafe", color: "#1e40af" }, Completed: { bg: "#dcfce7", color: "#166534" }, Cancelled: { bg: "#fee2e2", color: "#991b1b" } };

  const periodStats = (() => { if (!analytics) return { revenue: 0, orders: 0, label: "" }; if (timePeriod === "today") return { revenue: analytics.today.revenue, orders: analytics.today.orders, label: "Today" }; if (timePeriod === "7d") return { revenue: analytics.week.revenue, orders: analytics.week.orders, label: "Last 7 days" }; return { revenue: analytics.month.revenue, orders: analytics.month.orders, label: "This month" }; })();
  const trendPct = (() => { if (!analytics) return 0; const weekRev = analytics.week.revenue; const allRev = analytics.allTime.revenue; const prior = allRev - weekRev; const priorWk = prior / Math.max(1, analytics.allTime.orders - analytics.week.orders) * analytics.week.orders; return priorWk > 0 ? Math.round(((weekRev - priorWk) / priorWk) * 100) : 0; })();
  const trendUp = trendPct >= 0;
  const BAR_COLORS = analytics ? analytics.dailyChart.map((_, i) => i === analytics.dailyChart.length - 1 ? "#166534" : "#86efac") : [];
  const pieData = analytics ? [{ name: "Online", value: analytics.today.onlineOrders, fill: "#3b82f6" }, { name: "Walk-in", value: analytics.today.walkinOrders, fill: "#f59e0b" }] : [];
  ...
  const updateAptStatus = async (aptId, newStatus) => {
    try {
      ...
      setAptStatusDialogOpen(false); setSelectedApt(null);
    } catch { setNotification({ open: true, message: "Failed to update appointment", severity: "error" }); }
  };
  ...
  const addMedicine = async () => {
    if (!medForm.name || !medForm.price) { setNotification({ open: true, message: "Name and price are required", severity: "error" }); return; }
    try {
      ...
      setMedForm({ name: "", desc: "", price: "", category: "", img: "", stock: "100", lowStockThreshold: "10", unit: "units", supplier: "", expiryDate: "", entryDate: new Date().toISOString().split("T")[0] });
      setImgPreview("");
      axios.get(`${BASE_URL}/medicines/all`, { withCredentials: true }).then((r) => setMedicines(sanitizeObjectArray(r.data))).catch(() => {});
    } catch (err) { setNotification({ open: true, message: err?.response?.data?.message || "Failed to add medicine", severity: "error" }); }
  };
  ...
  const saveEditMedicine = async () => {
    try {
      ...
      axios.get(`${BASE_URL}/medicines/all`, { withCredentials: true }).then((r) => setMedicines(sanitizeObjectArray(r.data))).catch(() => {});
    } catch { setNotification({ open: true, message: "Failed to update medicine", severity: "error" }); }
  };
  ...
  const deleteMedicine = async (id) => {
    ...
    try {
      ...
      setMedicines((prev) => prev.filter((m) => m._id !== id)); setMedEditOpen(false);
    } catch { setNotification({ open: true, message: "Failed to remove medicine", severity: "error" }); }
  };
  ...
  const permanentDeleteMedicine = async (id, name) => {
    ...
    try {
      ...
      setMedicines((prev) => prev.filter((m) => m._id !== id)); setMedEditOpen(false);
    } catch { setNotification({ open: true, message: "Failed to delete medicine", severity: "error" }); }
  };

  const openStockUpdate = (med) => { setStockMed(med); setStockValue(""); setStockOperation("add"); setStockUpdateOpen(true); };

  const saveStockUpdate = async () => {
    if (!stockValue || isNaN(stockValue)) { setNotification({ open: true, message: "Enter a valid number", severity: "error" }); return; }
    try {
      const res = await axios.patch(`${BASE_URL}/medicines/${stockMed._id}/stock`, { stock: Number(stockValue), operation: stockOperation }, { withCredentials: true });
    ...
    } catch { setNotification({ open: true, message: "Failed to update stock", severity: "error" }); }
  };
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    ...
    } catch { setNotification({ open: true, message: "Failed to update status", severity: "error" }); }
  };
  ...
  const generateReceipt = (order, isWalkIn) => {
    ...
    w.document.write(`<html><head><title>Receipt #${id}</title></head><body style='font-family:Arial,sans-serif;padding:30px;max-width:600px;margin:auto;'><h2 style='color:#166534;text-align:center;'>Digital Clinic</h2><p style='text-align:center;color:#888;'>Order Receipt</p><hr/><p><strong>Order ID:</strong> #${id}</p><p><strong>Date:</strong> ${date}</p><p><strong>Customer:</strong> ${name}</p>${isWalkIn?`<p><strong>Type:</strong> Walk-in</p>`:""}<p><strong>Payment:</strong> ${order.paymentMethod||"Cash"}</p><p><strong>Status:</strong> ${order.status||"Pending"}</p><table style='width:100%;border-collapse:collapse;margin-top:15px;'><thead><tr style='background:#f0fdf4;'><th style='padding:8px;text-align:left;'>Medicine</th><th style='padding:8px;text-align:left;'>Price</th><th style='padding:8px;text-align:left;'>Qty</th></tr></thead><tbody>${rows}</tbody></table><h3 style='text-align:right;'>Total: Rs.${order.total}</h3><hr/><p style='text-align:center;color:#888;font-size:12px;'>Thank you for choosing Digital Clinic!</p><script>window.onload=function(){window.print();}</script></body></html>`);
    w.document.close();
  };

  // ─── USER MANAGEMENT ───────────────────────────────────────────────────────
  const createUser = async () => {
    if (!userForm.name||!userForm.email||!userForm.password) { setNotification({ open: true, message: "Name, email and password are required", severity: "error" }); return; }
    ...
    try {
      ...
      setUserForm({ name: "", email: "", phone: "", password: "", role: "user" }); setUserFormOpen(false);
      fetch(`${BASE_URL}/users`, { credentials: "include" }).then((r) => r.ok?r.json():[]).then((p) => setUsers(sanitizeObjectArray(p))).catch(() => {});
    } catch (err) { setNotification({ open: true, message: err?.response?.data?.message||"Failed to create user", severity: "error" }); }
    finally { setUserFormLoading(false); }
  };

  const updateUser = async () => {
    if (!editingUser) return; setUserFormLoading(true);
    try {
      const payload = {};
      ...
      setNotification({ open: true, message: "User updated!", severity: "success" }); setEditingUser(null);
      fetch(`${BASE_URL}/users`, { credentials: "include" }).then((r) => r.ok?r.json():[]).then((p) => setUsers(sanitizeObjectArray(p))).catch(() => {});
    } catch (err) { setNotification({ open: true, message: err?.response?.data?.message||"Failed to update user", severity: "error" }); }
    finally { setUserFormLoading(false); }
  };
  ...
  const deleteUser = async (userId, userName) => {
    ...
    } catch (err) { setNotification({ open: true, message: err?.response?.data?.message||"Failed to delete user", severity: "error" }); }
  };
  ...
  const toggleDisableUser = async (userId, isCurrentlyDisabled, userName) => {
    ...
    } catch (err) { setNotification({ open: true, message: err?.response?.data?.message||"Failed to toggle user status", severity: "error" }); }
  };

  // ─── NOTICE FUNCTIONS ──────────────────────────────────────────────────────
  const updateNotice = async () => {
    try {
      await fetch(`${BASE_URL}/notice`, { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include", body: JSON.stringify({ message: notice, expiresInHours: noticeHours }) });
      setNotification({ open: true, message: "Notice updated", severity: "success" }); setNotice(""); setNoticeHours("");
    } catch { setNotification({ open: true, message: "Error updating notice", severity: "error" }); }
  };
  const clearNotice = async () => {
    try { await fetch(`${BASE_URL}/notice`, { method: "DELETE", credentials: "include" }); setNotification({ open: true, message: "Notice deleted", severity: "success" }); }
  ...
  const posChangeQty = (id, delta) => setPosCart(posCart.map((i) => { if (i._id!==id) return i; const q=(i.quantity||1)+delta; return q<=0?null:{ ...i, quantity: q }; }).filter(Boolean));
  const posTotal = posCart.reduce((s, i) => s+Number(i.price||0)*(i.quantity||1), 0);
  const posFilteredMedicines = safeMedicines.filter((m) => { if (!m.isActive) return false; if (!posSearch.trim()) return true; return String(m.name||"").toLowerCase().includes(posSearch.toLowerCase()); });
  const posPlaceOrder = async () => {
    ...
    setPosPlacing(true);
    try {
      const res = await axios.post(`${BASE_URL}/orders/walk-in`, { items: posCart.map((i) => ({ name: i.name, price: i.price, quantity: i.quantity||1, img: i.img||"" })), total: posTotal, paymentMethod: posPaymentMethod, guestName: posCustomerName, guestPhone: posCustomerPhone, existingUserId: posMatchedUser ? posMatchedUser._id : null }, { withCredentials: true });
      setNotification({ open: true, message: "Walk-in order created!", severity: "success" });
      generateReceipt(res.data.order, true);
      setPosCart([]); setPosCustomerName(""); setPosCustomerPhone(""); setPosPaymentMethod("cash"); setPosMatchedUser(null); setPosSearch("");
      axios.get(`${BASE_URL}/medicines/all`, { withCredentials: true }).then((r) => setMedicines(sanitizeObjectArray(r.data))).catch(() => {});
      axios.get(`${BASE_URL}/orders`, { withCredentials: true }).then((r) => setOrders(sanitizeObjectArray(r.data))).catch(() => {});
    } catch (err) { setNotification({ open: true, message: err?.response?.data?.message||"Failed to create order", severity: "error" }); }
  ...
  return (
    <div style={styles.page}>
          ...
          {lowStockMeds.length > 0 && (
            <Tooltip title={`${lowStockMeds.length} low stock alert(s)`}>
              <IconButton onClick={() => setActiveTab("inventory")} style={{ background: "rgba(255,255,255,0.15)", backdropFilter: "blur(4px)" }}>
                <Badge badgeContent={lowStockMeds.length} color="warning"><Inventory style={{ color: "white" }} /></Badge>
              </IconButton>
            </Tooltip>
          )}
          <Tooltip title="New orders">
            <IconButton onClick={() => { setNewOrdersCount(0); setActiveTab("orders"); }} style={{ background: newOrdersCount>0?"rgba(239,68,68,0.2)":"rgba(255,255,255,0.15)", backdropFilter: "blur(4px)" }}>
              <Badge badgeContent={newOrdersCount} color="error"><NotificationsActive style={{ color: "white" }} /></Badge>
            </IconButton>
          </Tooltip>
      ...
      <div style={styles.tabBar}>
        {tabs.map((tab) => (
          <button key={tab.id} className="tab-btn" onClick={() => setActiveTab(tab.id)}
            ...
            {tab.label}
            {tab.id==="orders" && newOrdersCount>0 && <span style={{ marginLeft: "6px", background: "#dc2626", color: "white", borderRadius: "10px", padding: "1px 6px", fontSize: "10px", fontWeight: "700" }}>{newOrdersCount}</span>}
          </button>
        ))}
      </div>

      {/* ✅ ALL TABS ARE INSIDE styles.content — including Activity Log */}
      <div style={styles.content}>
          ...
          <div style={{ animation: "slideIn 0.3s ease" }}>
            ...
            <div style={styles.statsGrid}>
              {[
                ...
                <div key={i} className="metric-card" style={{ ...styles.statCard, borderTop: `4px solid ${color}`, transition: "all 0.2s ease", cursor: "default" }}>
                  <div style={{ width: "36px", height: "36px", borderRadius: "10px", background: bg, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "12px", color }}>{icon}</div>
            ...
            <div style={styles.card}>
              ...
              <div style={styles.statusGrid}>
                {Object.entries(ordersByStatus).map(([status, count]) => {
                  ...
                  return (
                    <div key={status} style={{ ...styles.statusBox, background: sc.bg }} onClick={() => setActiveTab("orders")}>
              ...
              <TableContainer component={Paper} elevation={0} style={{ border: "1px solid #e5e7eb", borderRadius: "10px" }}>
                <Table size="small">
                  <TableHead style={{ background: "#f9fafb" }}>
                    <TableRow>{["Order ID","Customer","Amount","Status","Date"].map((h) => <TableCell key={h}><strong style={{ fontSize: "12px", color: "#374151" }}>{h}</strong></TableCell>)}</TableRow>
                  </TableHead>
                  <TableBody>
                    {recentOrders.map((order) => {
                      ...
                      return (
                        <TableRow key={order._id} hover>
                          ...
                          <TableCell><span style={{ fontWeight: "600", fontSize: "13px" }}>{order.orderType==="walk-in"?(order.guestInfo?.name||"Walk-in"):(order.userId?.name||"Unknown")}</span>{order.orderType==="walk-in"&&<Chip label="Walk-in" size="small" style={{ marginLeft: "6px", background: "#fef3c7", color: "#92400e", fontSize: "10px", height: "18px" }} />}</TableCell>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              <div style={styles.card}>
                ...
                {[["Total Revenue",`Rs.${totalRevenue.toLocaleString()}`,"#166534",true],["Delivered Orders",ordersByStatus.Delivered,"#111",false],["Pending Orders",ordersByStatus.Pending,"#92400e",false],["Avg Order Value",`Rs.${totalOrders>0?Math.round(totalRevenue/totalOrders).toLocaleString():0}`,"#111",false]].map(([label,value,color,bold]) => (
              ...
              <div style={styles.card}>
                ...
                {[["Total Medicines",safeMedicines.length,"#166534",true],["Active in Store",safeMedicines.filter(m=>m.isActive).length,"#111",false],["Low Stock",lowStockMeds.filter(m=>m.stock>0).length,"#92400e",false],["Out of Stock",lowStockMeds.filter(m=>m.stock<=0).length,"#dc2626",false]].map(([label,value,color,bold]) => (
                  <div key={label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "1px solid #f3f4f6" }}>
                    <span style={{ fontSize: "14px", color: "#555" }}>{label}</span>
                    <span style={{ fontWeight: bold?"700":"600", color, fontSize: bold?"16px":"14px" }}>{value}</span>
                  </div>
        ...
        {/* ANALYTICS — keeping existing code, abbreviated for space */}
        {activeTab === "analytics" && (
          <div style={{ animation: "slideIn 0.3s ease" }}>
            {analyticsLoading ? (
              <div style={{ background: "white", borderRadius: "14px", padding: "80px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
                <div style={{ width: "36px", height: "36px", border: "4px solid #dcfce7", borderTop: "4px solid #166534", borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto 16px" }} />
                <p style={{ color: "#166534" }}>Loading analytics...</p>
              </div>
            ) : !analytics ? (
              <div style={{ background: "white", borderRadius: "16px", padding: "80px 32px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
                ...
                <button onClick={() => setActiveTab("pos")} style={{ padding: "11px 28px", background: "#166534", color: "white", border: "none", borderRadius: "10px", fontWeight: "600", cursor: "pointer", fontSize: "14px" }}>Create Walk-in Order →</button>
              </div>
            ) : (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px", flexWrap: "wrap", gap: "12px" }}>
                  <h2 style={{ fontSize: "22px", fontWeight: "700", color: "#111", margin: 0 }}>Sales Analytics</h2>
                  <div style={{ display: "flex", gap: "10px" }}>
                    <div style={{ display: "flex", background: "#f3f4f6", borderRadius: "10px", padding: "4px", gap: "2px" }}>
                      {[["today","Today"],["7d","7 days"],["month","Month"]].map(([val,label]) => (
                        <button key={val} onClick={() => setTimePeriod(val)} style={{ padding: "7px 16px", border: "none", borderRadius: "7px", cursor: "pointer", fontSize: "13px", fontWeight: "600", background: timePeriod===val?"white":"transparent", color: timePeriod===val?"#166534":"#6b7280", boxShadow: timePeriod===val?"0 1px 4px rgba(0,0,0,0.14)":"none" }}>{label}</button>
                      ))}
                    </div>
                    <button onClick={fetchAnalytics} style={{ padding: "8px 16px", background: "white", color: "#166534", border: "1px solid #d1d5db", borderRadius: "9px", fontSize: "13px", fontWeight: "600", cursor: "pointer" }}>↻ Refresh</button>
                  </div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: "16px", marginBottom: "24px" }}>
                  {[
                    { label: `${periodStats.label} Revenue`, value: `Rs.${periodStats.revenue.toLocaleString()}`, color: "#166534", border: "#166534" },
                    { label: `${periodStats.label} Orders`,  value: periodStats.orders,                          color: "#1e40af", border: "#3b82f6" },
                    { label: "This Month",                   value: `Rs.${analytics.month.revenue.toLocaleString()}`, color: "#6d28d9", border: "#8b5cf6" },
                    { label: "All-time",                     value: `Rs.${analytics.allTime.revenue.toLocaleString()}`, color: "#b45309", border: "#f59e0b" },
                  ].map(({ label, value, color, border }) => (
                    <div key={label} style={{ background: "white", borderRadius: "14px", padding: "20px", boxShadow: "0 2px 8px rgba(0,0,0,0.07)", borderTop: `4px solid ${border}` }}>
                ...
                <div style={{ background: "white", borderRadius: "14px", padding: "28px", boxShadow: "0 2px 8px rgba(0,0,0,0.07)" }}>
                  <h3 style={{ margin: "0 0 24px", fontSize: "17px", fontWeight: "700" }}>Revenue — last 7 days</h3>
                  <div style={{ height: "300px" }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={analytics.dailyChart} barCategoryGap="35%">
                        <CartesianGrid strokeDasharray="4 4" stroke="#f0f0f0" vertical={false} />
                        <XAxis dataKey="date" tick={{ fontSize: 12, fill: "#9ca3af" }} tickLine={false} />
                        <YAxis tick={{ fontSize: 12, fill: "#9ca3af" }} tickFormatter={(v) => `Rs.${v>=1000?Math.round(v/1000)+"k":v}`} axisLine={false} tickLine={false} width={76} />
                        <ReTooltip formatter={(v) => [`Rs.${Number(v).toLocaleString()}`, "Revenue"]} contentStyle={{ borderRadius: "10px", border: "1px solid #e5e7eb", fontSize: "13px" }} cursor={{ fill: "rgba(22,101,52,0.04)" }} />
        ...
        {activeTab === "queue" && (
          <div style={{ animation: "slideIn 0.3s ease" }}>
            <div style={{ marginBottom: "24px" }}>
              ...
              <p style={{ fontSize: "13px", color: "#9ca3af", margin: 0 }}>Click "Next Patient" — current appointment auto-completes, next is called.</p>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: "20px" }}>
              {[
                ...
                const q=queueStatus[type]||{}; const total=q.totalIssued||0; const serving=q.currentServing||0; const waiting=Math.max(0,total-serving); const isLoading=queueLoading[type];
                return (
                  <div key={type} style={{ background: "white", borderRadius: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.07)", overflow: "hidden" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "1px", background: "#f3f4f6" }}>
                      {[{ label: "Now Serving", value: serving||"—", highlight: true },{ label: "Waiting", value: waiting },{ label: "Total Today", value: total }].map(({ label: sl, value, highlight }) => (
                    ...
                    {serving>0&&<div style={{ margin: "0 20px 20px", background: bg, border: `1px solid ${border}`, borderRadius: "10px", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}><span style={{ fontSize: "13px", color, fontWeight: "600" }}>Now calling:</span><span style={{ fontSize: "20px", fontWeight: "700", color, letterSpacing: "0.05em" }}>{type==="appointment"?"APT":type==="walkin"?"WLK":"ORD"}-{String(serving).padStart(3,"0")}</span></div>}
                  </div>
                );
              })}
            </div>
        ...
        {activeTab === "orders" && (
          <div style={{ ...styles.card, animation: "slideIn 0.3s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
              <div><h3 style={styles.cardTitle}>📦 Order Management</h3><p style={{ margin: "4px 0 0", fontSize: "13px", color: "#9ca3af" }}>{filteredOrders.length} orders total</p></div>
              <TextField size="small" placeholder="Search by name, email, status, ID..." value={orderSearch} onChange={(e) => setOrderSearch(e.target.value)} InputProps={{ startAdornment: <InputAdornment position="start"><Search fontSize="small" /></InputAdornment> }} style={{ minWidth: "280px" }} />
            </div>
            <TableContainer component={Paper} elevation={0} style={{ border: "1px solid #e5e7eb", borderRadius: "10px" }}>
              <Table>
                <TableHead style={{ background: "#f9fafb" }}>
                  <TableRow>{["Order ID","Customer","Type","Items","Amount","Payment","Status","Date","Actions"].map((h) => <TableCell key={h}><strong style={{ fontSize: "12px", color: "#374151" }}>{h}</strong></TableCell>)}</TableRow>
                </TableHead>
                <TableBody>
                  {filteredOrders.map((order) => {
                    ...
                    return (
                      <TableRow key={order._id} hover>
                        <TableCell style={{ fontWeight: "700", color: "#166534", fontSize: "13px" }}>#{order._id?.toString().slice(-6).toUpperCase()}</TableCell>
                        <TableCell><div style={{ fontWeight: "600", fontSize: "13px" }}>{cName}</div><div style={{ fontSize: "11px", color: "#9ca3af" }}>{cSub}</div></TableCell>
                        <TableCell><Chip label={order.orderType==="walk-in"?"🏪 Walk-in":"🌐 Online"} size="small" style={{ background: order.orderType==="walk-in"?"#fef3c7":"#dbeafe", color: order.orderType==="walk-in"?"#92400e":"#1e40af", fontSize: "11px", fontWeight: "600" }} /></TableCell>
                        <TableCell>{(Array.isArray(order.items)?order.items:[]).slice(0,2).map((item,i) => <div key={i} style={{ fontSize: "12px", color: "#555" }}>• {item.name}</div>)}{Array.isArray(order.items)&&order.items.length>2&&<div style={{ fontSize: "11px", color: "#9ca3af" }}>+{order.items.length-2} more</div>}</TableCell>
                        <TableCell><strong style={{ color: "#166534" }}>Rs.{order.total}</strong></TableCell>
                        ...
                        <TableCell>
                          <Tooltip title="Update Status"><IconButton size="small" style={{ color: "#166534" }} onClick={() => { setSelectedOrder(order); setStatusDialogOpen(true); }}><Edit fontSize="small" /></IconButton></Tooltip>
        ...
        {activeTab === "appointments" && (
          <div style={{ ...styles.card, animation: "slideIn 0.3s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
              <div><h3 style={styles.cardTitle}>📅 Appointment Management</h3><p style={{ color: "#9ca3af", fontSize: "13px", margin: "4px 0 0" }}>{safeAppointments.length} total · {safeAppointments.filter(a=>a.status==="Pending").length} pending</p></div>
              <TextField size="small" placeholder="Search by name, contact, problem..." value={aptSearch} onChange={(e) => setAptSearch(e.target.value)} InputProps={{ startAdornment: <InputAdornment position="start"><Search fontSize="small" /></InputAdornment> }} style={{ minWidth: "280px" }} />
            </div>
            {filteredApts.length===0 ? (
              <div style={{ textAlign: "center", padding: "48px", color: "#9ca3af" }}><div style={{ fontSize: "40px", marginBottom: "12px" }}>📅</div><p style={{ fontSize: "14px" }}>No appointments found</p></div>
            ) : (
              <TableContainer component={Paper} elevation={0} style={{ border: "1px solid #e5e7eb", borderRadius: "10px" }}>
                <Table>
                  <TableHead style={{ background: "#f9fafb" }}>
                    <TableRow>{["Patient","Age","Contact","Problem","Date & Time","Booked On","Status","Action"].map((h) => <TableCell key={h}><strong style={{ fontSize: "12px", color: "#374151" }}>{h}</strong></TableCell>)}</TableRow>
                  </TableHead>
                  <TableBody>
                    {filteredApts.map((apt, idx) => {
                      ...
                      return (
                        <TableRow key={apt._id||idx} hover>
                          <TableCell><div style={{ display: "flex", alignItems: "center", gap: "10px" }}><div style={{ width: "32px", height: "32px", borderRadius: "50%", background: "linear-gradient(135deg,#166534,#4ade80)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: "700", fontSize: "13px", flexShrink: 0 }}>{(apt.name||"?").charAt(0).toUpperCase()}</div><span style={{ fontWeight: "600", fontSize: "13px" }}>{apt.name||"-"}</span></div></TableCell>
                          <TableCell style={{ fontSize: "13px" }}>{apt.age||"-"}</TableCell>
                          <TableCell style={{ color: "#555", fontSize: "13px" }}>{apt.contact||"-"}</TableCell>
                          <TableCell style={{ maxWidth: "180px" }}><div style={{ fontSize: "13px", color: "#555", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{apt.problem||"-"}</div></TableCell>
                          <TableCell><div style={{ fontWeight: "600", color: "#166534", fontSize: "13px" }}>{apt.date||"-"}</div><div style={{ fontSize: "11px", color: "#9ca3af" }}>{apt.time||"-"}</div></TableCell>
                          ...
                          <TableCell><Tooltip title="Update Status"><IconButton size="small" style={{ color: "#166534" }} onClick={() => { setSelectedApt({ ...apt }); setAptStatusDialogOpen(true); }}><Edit fontSize="small" /></IconButton></Tooltip></TableCell>
                        </TableRow>
                      );
                    })}
                  </TableBody>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: "24px", alignItems: "start", animation: "slideIn 0.3s ease" }}>
            <div style={styles.card}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "20px" }}>
                <div><label style={styles.fieldLabel}>Customer Name *</label><input placeholder="Enter customer name" value={posCustomerName} onChange={(e) => setPosCustomerName(e.target.value)} style={styles.inputField} /></div>
                <div>
                  <label style={styles.fieldLabel}>Phone Number</label>
                  <input placeholder="Enter phone to check account" value={posCustomerPhone} onChange={(e) => { setPosCustomerPhone(e.target.value); searchUserByPhone(e.target.value); }} style={styles.inputField} />
              ...
              <div style={{ marginBottom: "20px" }}>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  {[["cash","💵"],["upi","📱"],["card","💳"]].map(([method,icon]) => (
                    <button key={method} onClick={() => setPosPaymentMethod(method)} style={{ padding: "9px 22px", borderRadius: "9px", border: "2px solid", borderColor: posPaymentMethod===method?"#166534":"#e5e7eb", background: posPaymentMethod===method?"#166534":"white", color: posPaymentMethod===method?"white":"#555", fontWeight: "600", cursor: "pointer", fontSize: "14px" }}>{icon} {method}</button>
                  ))}
                </div>
              </div>
              <div style={{ marginBottom: "16px" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: "9px", padding: "0 14px" }}>
                  ...
                  <input placeholder="Search medicine name..." value={posSearch} onChange={(e) => setPosSearch(e.target.value)} style={{ flex: 1, border: "none", background: "transparent", padding: "10px 0", fontSize: "14px", outline: "none" }} />
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(155px,1fr))", gap: "10px", maxHeight: "420px", overflowY: "auto" }}>
                {posFilteredMedicines.map((m) => {
                  const inCart=posCart.find((i) => i._id===m._id); const oos=m.stock<=0;
                  return (
                    <div key={m._id} onClick={() => !oos&&posAddToCart(m)} style={{ border: inCart?"2px solid #166534":"1px solid #e5e7eb", borderRadius: "10px", padding: "12px", cursor: oos?"not-allowed":"pointer", background: oos?"#f9fafb":inCart?"#f0fdf4":"white", opacity: oos?0.55:1, position: "relative" }}>
                      ...
                      <div style={{ fontSize: "11px", color: oos?"#dc2626":m.stock<=m.lowStockThreshold?"#92400e":"#9ca3af" }}>{oos?"Out of stock":`Stock: ${m.stock}`}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ position: "sticky", top: "20px" }}>
              <div style={{ ...styles.card, border: "2px solid #166534" }}>
                ...
                {posCart.length===0 ? (
                  <div style={{ textAlign: "center", padding: "36px 20px", color: "#9ca3af" }}><div style={{ fontSize: "40px", marginBottom: "10px" }}>🛒</div><p style={{ fontSize: "14px" }}>Click a medicine to add it</p></div>
                ) : (
                  <div>
                    {posCart.map((item) => (
                      <div key={item._id} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "10px 0", borderBottom: "1px solid #f3f4f6" }}>
                        <div style={{ flex: 1 }}><div style={{ fontSize: "13px", fontWeight: "600" }}>{item.name}</div><div style={{ fontSize: "12px", color: "#9ca3af" }}>Rs.{item.price} each</div></div>
                    ...
                    {posCustomerName&&<div style={{ background: "#f0fdf4", borderRadius: "9px", padding: "10px 14px", marginBottom: "14px", fontSize: "13px" }}><div><strong>Customer:</strong> {posCustomerName}</div>{posCustomerPhone&&<div><strong>Phone:</strong> {posCustomerPhone}</div>}<div><strong>Payment:</strong> {posPaymentMethod}</div>{posMatchedUser&&<div style={{ color: "#166534", marginTop: "4px" }}>✅ Linked to account</div>}</div>}
                    <button onClick={posPlaceOrder} disabled={posPlacing} style={{ width: "100%", padding: "14px", background: posPlacing?"#9ca3af":"#166534", color: "white", border: "none", borderRadius: "10px", fontWeight: "700", fontSize: "15px", cursor: posPlacing?"not-allowed":"pointer" }}>{posPlacing?"⏳ Creating order...":"✅ Complete Sale & Print Receipt"}</button>
                    <button onClick={() => { setPosCart([]); setPosCustomerName(""); setPosCustomerPhone(""); setPosMatchedUser(null); }} style={{ width: "100%", padding: "10px", background: "none", border: "1px solid #e5e7eb", borderRadius: "8px", color: "#9ca3af", cursor: "pointer", marginTop: "8px", fontSize: "14px" }}>Clear All</button>
                  </div>
                )}
              </div>
            </div>
        ...
        {/* INVENTORY — ✅ includes supplier, expiry date, entry date */}
        {activeTab === "inventory" && (
          <div style={{ animation: "slideIn 0.3s ease" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: "14px", marginBottom: "24px" }}>
              {[
                ...
                <div key={label} style={{ background: "white", borderRadius: "12px", padding: "16px 18px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", borderLeft: `4px solid ${color}`, display: "flex", alignItems: "center", gap: "12px" }}>
                  ...
                  <div><div style={{ fontSize: "22px", fontWeight: "700", color }}>{value}</div><div style={{ fontSize: "12px", color: "#6b7280" }}>{label}</div></div>
                </div>
              ))}
            </div>
            ...
            {/* ✅ ADD MEDICINE FORM — includes supplier, expiryDate, entryDate */}
            <div style={styles.card}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginTop: "16px" }}>
                ...
                {/* ✅ NEW FIELDS */}
                <div><label style={styles.fieldLabel}>Supplier / Company</label><input placeholder="e.g. Sun Pharma, Cipla" value={medForm.supplier} onChange={(e) => setMedForm({ ...medForm, supplier: e.target.value })} style={styles.inputField} /></div>
                <div>
                  <label style={styles.fieldLabel}>Expiry Date</label>
                  <input type="date" value={medForm.expiryDate} onChange={(e) => setMedForm({ ...medForm, expiryDate: e.target.value })} style={styles.inputField} />
                </div>
                <div>
                  <label style={styles.fieldLabel}>Entry Date</label>
                  <input type="date" value={medForm.entryDate} onChange={(e) => setMedForm({ ...medForm, entryDate: e.target.value })} style={styles.inputField} />
                </div>
            ...
            {/* MEDICINES TABLE — ✅ shows supplier, expiry, entry date */}
            <div style={styles.card}>
              ...
              <TableContainer component={Paper} elevation={0} style={{ border: "1px solid #e5e7eb", borderRadius: "10px", marginTop: "16px" }}>
                <Table>
                  <TableHead style={{ background: "#f9fafb" }}>
                    <TableRow>
                      {["Medicine","Category","Supplier","Price","Stock","Expiry","Entry Date","Status","Visibility","Actions"].map((h) => (
                        ...
                        <TableRow key={med._id} hover style={{ opacity: med.isActive?1:0.5 }}>
                          <TableCell>
                            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                              ...
                              <div><div style={{ fontWeight: "600", fontSize: "13px" }}>{med.name}</div><div style={{ fontSize: "11px", color: "#9ca3af" }}>{med.desc?.slice(0,38)||""}</div></div>
                            </div>
                          </TableCell>
                          <TableCell style={{ fontSize: "13px", color: "#555" }}>{med.category||"-"}</TableCell>
                          <TableCell style={{ fontSize: "12px", color: "#555" }}>{med.supplier||<span style={{ color: "#d1d5db" }}>—</span>}</TableCell>
                          <TableCell><strong style={{ color: "#166534" }}>Rs.{med.price}</strong></TableCell>
                          <TableCell><div style={{ fontWeight: "700", fontSize: "15px", color: ss.color }}>{med.stock}</div><div style={{ fontSize: "11px", color: "#9ca3af" }}>{med.unit||"units"}</div></TableCell>
                          <TableCell>
                            {expSt ? (
                              <span style={{ display: "inline-block", background: expSt.bg, color: expSt.color, padding: "2px 8px", borderRadius: "8px", fontSize: "11px", fontWeight: "600" }}>
                          ...
                          <TableCell style={{ fontSize: "12px", color: "#555" }}>
                            {med.entryDate ? new Date(med.entryDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "2-digit" }) : <span style={{ color: "#d1d5db" }}>—</span>}
                          </TableCell>
                          ...
                          <TableCell>
                            ...
                            <button onClick={() => axios.put(`${BASE_URL}/medicines/${med._id}`, { isActive: !med.isActive }, { withCredentials: true }).then(() => setMedicines((prev) => prev.map((m) => m._id===med._id ? { ...m, isActive: !med.isActive } : m)))} style={{ padding: "4px 10px", background: med.isActive?"#fee2e2":"#dcfce7", color: med.isActive?"#991b1b":"#166534", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "600", fontSize: "12px", margin: "0 4px" }}>{med.isActive?"Hide":"Show"}</button>
                            <Tooltip title="Permanently delete from database">
                              <button onClick={() => permanentDeleteMedicine(med._id, med.name)} style={{ padding: "4px 10px", background: "#fee2e2", color: "#991b1b", border: "1px solid #fecaca", borderRadius: "6px", cursor: "pointer", fontWeight: "600", fontSize: "12px" }}>🗑️ Delete</button>
                            </Tooltip>
                          </TableCell>
                        </TableRow>
                      );
        ...
        {activeTab === "users" && (
          <div style={{ animation: "slideIn 0.3s ease" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
              <div><h3 style={styles.cardTitle}>👥 User Management</h3><p style={{ margin: "4px 0 0", fontSize: "13px", color: "#9ca3af" }}>{safeUsers.length} total users</p></div>
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                <TextField size="small" placeholder="Search users..." value={userSearch} onChange={(e) => setUserSearch(e.target.value)} InputProps={{ startAdornment: <InputAdornment position="start"><Search fontSize="small" /></InputAdornment> }} />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: "12px", marginBottom: "20px" }}>
              {[{ role: "admin", label: "Admins", icon: "🔑", color: "#1e40af" },{ role: "staff", label: "Staff", icon: "🏥", color: "#166534" },{ role: "reception", label: "Reception", icon: "🖥️", color: "#6d28d9" },{ role: "user", label: "Patients", icon: "👤", color: "#92400e" }].map(({ role, label, icon, color }) => (
                <div key={role} style={{ background: "white", borderRadius: "10px", padding: "14px 16px", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", borderLeft: `4px solid ${color}`, display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <div><div style={{ fontSize: "18px", fontWeight: "700", color }}>{safeUsers.filter(u=>u.role===role).length}</div><div style={{ fontSize: "11px", color: "#9ca3af" }}>{label}</div></div>
                </div>
              ))}
            </div>
            <div style={styles.card}>
              <TableContainer component={Paper} elevation={0} style={{ border: "1px solid #e5e7eb", borderRadius: "10px" }}>
                <Table>
                  <TableHead style={{ background: "#f9fafb" }}>
                    <TableRow>{["Name","Email","Phone","Role","Status","Joined","Actions"].map((h) => <TableCell key={h}><strong style={{ fontSize: "12px", color: "#374151" }}>{h}</strong></TableCell>)}</TableRow>
                  </TableHead>
                  <TableBody>
                    {filteredUsers.map((user, idx) => (
                      <TableRow key={idx} hover style={{ opacity: user.isDisabled?0.5:1 }}>
                        <TableCell><div style={{ display: "flex", alignItems: "center", gap: "10px" }}><div style={{ width: "36px", height: "36px", borderRadius: "50%", background: user.isDisabled?"#e5e7eb":"linear-gradient(135deg,#166534,#4ade80)", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontWeight: "700", fontSize: "14px", flexShrink: 0 }}>{(user.name||"U").charAt(0).toUpperCase()}</div><div><div style={{ fontWeight: "600", fontSize: "13px" }}>{user.name||"-"}</div>{user.isDisabled&&<div style={{ fontSize: "10px", color: "#dc2626", fontWeight: "600" }}>DISABLED</div>}</div></div></TableCell>
                        <TableCell style={{ color: "#555", fontSize: "13px" }}>{user.email||"-"}</TableCell>
                        <TableCell style={{ color: "#555", fontSize: "13px" }}>{user.phone||"-"}</TableCell>
                        <TableCell><Chip label={user.role||"user"} size="small" style={{ background: user.role==="admin"?"#dbeafe":user.role==="staff"?"#dcfce7":user.role==="reception"?"#faf5ff":"#f9fafb", color: user.role==="admin"?"#1e40af":user.role==="staff"?"#166534":user.role==="reception"?"#6d28d9":"#374151", fontWeight: "600", fontSize: "11px" }} /></TableCell>
                        <TableCell><Chip label={user.isDisabled?"Disabled":"Active"} size="small" style={{ background: user.isDisabled?"#fee2e2":"#dcfce7", color: user.isDisabled?"#991b1b":"#166534", fontWeight: "600", fontSize: "11px" }} /></TableCell>
                        <TableCell style={{ fontSize: "12px", color: "#9ca3af" }}>{user.createdAt?new Date(user.createdAt).toLocaleDateString():"-"}</TableCell>
                        <TableCell>
                          <div style={{ display: "flex", gap: "4px", flexWrap: "wrap" }}>
                            <Tooltip title="Edit user"><IconButton size="small" style={{ color: "#166534" }} onClick={() => setEditingUser({ ...user, newPassword: "" })}><Edit fontSize="small" /></IconButton></Tooltip>
                            <button onClick={() => toggleDisableUser(user._id, user.isDisabled, user.name)} style={{ padding: "3px 8px", background: user.isDisabled?"#dcfce7":"#fef3c7", color: user.isDisabled?"#166534":"#92400e", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "11px", fontWeight: "600" }}>{user.isDisabled?"Enable":"Disable"}</button>
                            <button onClick={() => deleteUser(user._id, user.name)} style={{ padding: "3px 8px", background: "#fee2e2", color: "#991b1b", border: "1px solid #fecaca", borderRadius: "6px", cursor: "pointer", fontSize: "11px", fontWeight: "600" }}>🗑️</button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
        ...
        {activeTab === "notices" && (
          <div style={{ ...styles.card, animation: "slideIn 0.3s ease" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px", maxWidth: "580px" }}>
              <div><label style={styles.fieldLabel}>Notice Message</label><textarea value={notice} onChange={(e) => setNotice(e.target.value)} placeholder="Enter your notice message here..." rows={4} style={{ ...styles.inputField, width: "100%", resize: "vertical", lineHeight: "1.6" }} /></div>
              <div><label style={styles.fieldLabel}>Auto-delete after (hours)</label><input value={noticeHours} onChange={(e) => setNoticeHours(e.target.value)} placeholder="e.g. 24  —  leave empty for permanent" style={{ ...styles.inputField, width: "100%" }} /></div>
              <div style={{ display: "flex", gap: "12px" }}>
                <button style={styles.addBtn} onClick={updateNotice}>Publish Notice</button>
                <button onClick={clearNotice} style={{ padding: "10px 22px", background: "#fee2e2", color: "#991b1b", border: "none", borderRadius: "8px", cursor: "pointer", fontWeight: "600", fontSize: "14px" }}>Delete Notice</button>
              </div>
            </div>
          </div>
        )}

        {/* ✅ ACTIVITY LOG — correctly INSIDE styles.content div */}
        {activeTab === "activity" && (
          <ActivityLogTab
            logs={activityLogs}
            total={activityTotal}
      ...
      </div>{/* ✅ end styles.content — ALL tabs are inside here */}

      {/* DIALOGS */}
      <Dialog open={statusDialogOpen} onClose={() => setStatusDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle style={{ background: "#f0fdf4", color: "#166534", fontWeight: "700" }}>Update Order Status</DialogTitle>
        <DialogContent>{selectedOrder&&<Box sx={{ pt: 2 }}><Typography variant="body2" sx={{ mb: 2, color: "#555" }}>Order <strong>#{selectedOrder._id?.toString().slice(-6).toUpperCase()}</strong></Typography><FormControl fullWidth><InputLabel>New Status</InputLabel><Select value={selectedOrder.status||"Pending"} onChange={(e) => setSelectedOrder({ ...selectedOrder, status: e.target.value })} label="New Status"><MenuItem value="Pending">⏳ Pending</MenuItem><MenuItem value="Approved">✅ Approved</MenuItem><MenuItem value="Out for Delivery">🚚 Out for Delivery</MenuItem><MenuItem value="Delivered">📦 Delivered</MenuItem><MenuItem value="Cancelled">❌ Cancelled</MenuItem><MenuItem value="Completed">✔️ Completed</MenuItem></Select></FormControl></Box>}</DialogContent>
        <DialogActions style={{ padding: "16px" }}><Button onClick={() => setStatusDialogOpen(false)} style={{ color: "#888" }}>Cancel</Button><Button onClick={() => updateOrderStatus(selectedOrder._id, selectedOrder.status)} variant="contained" style={{ background: "#166534", color: "white" }}>Update</Button></DialogActions>
      </Dialog>
      ...
      <Dialog open={aptStatusDialogOpen} onClose={() => setAptStatusDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>{selectedApt&&<Box sx={{ pt: 2 }}><Typography variant="body2" sx={{ mb: 1, color: "#555" }}>Patient: <strong>{selectedApt.name}</strong></Typography><Typography variant="body2" sx={{ mb: 2, color: "#555" }}>Appointment: <strong>{selectedApt.date} at {selectedApt.time}</strong></Typography><FormControl fullWidth><InputLabel>New Status</InputLabel><Select value={selectedApt.status||"Pending"} onChange={(e) => setSelectedApt({ ...selectedApt, status: e.target.value })} label="New Status"><MenuItem value="Pending">⏳ Pending</MenuItem><MenuItem value="Confirmed">✅ Confirmed</MenuItem><MenuItem value="Completed">🏥 Completed</MenuItem><MenuItem value="Cancelled">❌ Cancelled</MenuItem></Select></FormControl></Box>}</DialogContent>
        <DialogActions style={{ padding: "16px" }}><Button onClick={() => setAptStatusDialogOpen(false)} style={{ color: "#888" }}>Cancel</Button><Button onClick={() => updateAptStatus(selectedApt._id, selectedApt.status)} variant="contained" style={{ background: "#166534", color: "white" }}>Update Status</Button></DialogActions>
      </Dialog>

      {/* ✅ EDIT MEDICINE DIALOG — includes supplier, expiry, entry date */}
      <Dialog open={medEditOpen} onClose={() => setMedEditOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          {editingMed&&<Box sx={{ pt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <TextField label="Supplier / Company" value={editingMed.supplier||""} onChange={(e) => setEditingMed({ ...editingMed, supplier: e.target.value })} fullWidth size="small" placeholder="e.g. Sun Pharma, Cipla" />
            ...
            <FormControl fullWidth size="small"><InputLabel>Unit</InputLabel><Select value={editingMed.unit||"units"} onChange={(e) => setEditingMed({ ...editingMed, unit: e.target.value })} label="Unit">{["units","bottles","strips","boxes","sachets","vials"].map((u) => <MenuItem key={u} value={u}>{u.charAt(0).toUpperCase()+u.slice(1)}</MenuItem>)}</Select></FormControl>
            ...
            <TextField label="Entry Date" type="date" value={editingMed.entryDate||""} onChange={(e) => setEditingMed({ ...editingMed, entryDate: e.target.value })} fullWidth size="small" InputLabelProps={{ shrink: true }} />
          </Box>}
        </DialogContent>
        <DialogActions style={{ padding: "16px", display: "flex", justifyContent: "space-between" }}>
          <Button onClick={() => editingMed&&deleteMedicine(editingMed._id)} style={{ color: "#dc2626", border: "1px solid #fecaca" }}>Delete Medicine</Button>
          <div style={{ display: "flex", gap: "8px" }}><Button onClick={() => setMedEditOpen(false)} style={{ color: "#888" }}>Cancel</Button><Button onClick={saveEditMedicine} variant="contained" style={{ background: "#166534", color: "white" }}>Save Changes</Button></div>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={stockUpdateOpen} onClose={() => setStockUpdateOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent><Box sx={{ pt: 2, display: "flex", flexDirection: "column", gap: 2 }}><Typography variant="body2" color="text.secondary">Current stock: <strong>{stockMed?.stock} {stockMed?.unit||"units"}</strong></Typography><FormControl fullWidth size="small"><InputLabel>Operation</InputLabel><Select value={stockOperation} onChange={(e) => setStockOperation(e.target.value)} label="Operation"><MenuItem value="add">➕ Add to stock</MenuItem><MenuItem value="subtract">➖ Remove from stock</MenuItem><MenuItem value="set">📝 Set exact value</MenuItem></Select></FormControl><TextField label={stockOperation==="add"?"Quantity to Add":stockOperation==="subtract"?"Quantity to Remove":"Set Stock To"} type="number" value={stockValue} onChange={(e) => setStockValue(e.target.value)} fullWidth size="small" />{stockValue&&!isNaN(stockValue)&&<Typography variant="body2" style={{ color: "#166534", background: "#f0fdf4", padding: "10px", borderRadius: "8px" }}>New stock will be: <strong>{stockOperation==="add"?Number(stockMed?.stock||0)+Number(stockValue):stockOperation==="subtract"?Math.max(0,Number(stockMed?.stock||0)-Number(stockValue)):Number(stockValue)} {stockMed?.unit||"units"}</strong></Typography>}</Box></DialogContent>
        <DialogActions style={{ padding: "16px" }}><Button onClick={() => setStockUpdateOpen(false)} style={{ color: "#888" }}>Cancel</Button><Button onClick={saveStockUpdate} variant="contained" style={{ background: "#166534", color: "white" }}>Update Stock</Button></DialogActions>
      </Dialog>
      ...
      <Dialog open={userFormOpen} onClose={() => setUserFormOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent><Box sx={{ pt: 2, display: "flex", flexDirection: "column", gap: 2 }}><TextField label="Full Name *" value={userForm.name} onChange={(e) => setUserForm((f) => ({ ...f, name: e.target.value }))} fullWidth size="small" /><TextField label="Email *" type="email" value={userForm.email} onChange={(e) => setUserForm((f) => ({ ...f, email: e.target.value }))} fullWidth size="small" /><TextField label="Phone" value={userForm.phone} onChange={(e) => setUserForm((f) => ({ ...f, phone: e.target.value }))} fullWidth size="small" /><TextField label="Password *" type="password" value={userForm.password} onChange={(e) => setUserForm((f) => ({ ...f, password: e.target.value }))} fullWidth size="small" helperText="Minimum 6 characters" /><FormControl fullWidth size="small"><InputLabel>Role</InputLabel><Select value={userForm.role} onChange={(e) => setUserForm((f) => ({ ...f, role: e.target.value }))} label="Role"><MenuItem value="admin">🔑 Admin — Full access</MenuItem><MenuItem value="staff">🏥 Staff — Orders & queue</MenuItem><MenuItem value="reception">🖥️ Reception — Token desk</MenuItem><MenuItem value="user">👤 Patient — Store & appointments</MenuItem></Select></FormControl></Box></DialogContent>
        <DialogActions style={{ padding: "16px" }}><Button onClick={() => setUserFormOpen(false)} style={{ color: "#888" }}>Cancel</Button><Button onClick={createUser} disabled={userFormLoading} variant="contained" style={{ background: "#166534", color: "white" }}>{userFormLoading?"Creating...":"Create User"}</Button></DialogActions>
      </Dialog>
      ...
      <Dialog open={!!editingUser} onClose={() => setEditingUser(null)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>{editingUser&&<Box sx={{ pt: 2, display: "flex", flexDirection: "column", gap: 2 }}><TextField label="Full Name" value={editingUser.name||""} onChange={(e) => setEditingUser((u) => ({ ...u, name: e.target.value }))} fullWidth size="small" /><TextField label="Email" value={editingUser.email||""} onChange={(e) => setEditingUser((u) => ({ ...u, email: e.target.value }))} fullWidth size="small" /><TextField label="Phone" value={editingUser.phone||""} onChange={(e) => setEditingUser((u) => ({ ...u, phone: e.target.value }))} fullWidth size="small" /><TextField label="New Password" type="password" value={editingUser.newPassword||""} onChange={(e) => setEditingUser((u) => ({ ...u, newPassword: e.target.value }))} fullWidth size="small" helperText="Leave blank to keep current password" /><FormControl fullWidth size="small"><InputLabel>Role</InputLabel><Select value={editingUser.role||"user"} onChange={(e) => setEditingUser((u) => ({ ...u, role: e.target.value }))} label="Role"><MenuItem value="admin">🔑 Admin</MenuItem><MenuItem value="staff">🏥 Staff</MenuItem><MenuItem value="reception">🖥️ Reception</MenuItem><MenuItem value="user">👤 Patient</MenuItem></Select></FormControl></Box>}</DialogContent>
        <DialogActions style={{ padding: "16px", display: "flex", justifyContent: "space-between" }}>
          <Button onClick={() => { deleteUser(editingUser._id, editingUser.name); setEditingUser(null); }} style={{ color: "#dc2626", border: "1px solid #fecaca" }}>🗑️ Delete User</Button>
          <div style={{ display: "flex", gap: "8px" }}><Button onClick={() => setEditingUser(null)} style={{ color: "#888" }}>Cancel</Button><Button onClick={updateUser} disabled={userFormLoading} variant="contained" style={{ background: "#166534", color: "white" }}>{userFormLoading?"Saving...":"Save Changes"}</Button></div>
        </DialogActions>
      </Dialog>
