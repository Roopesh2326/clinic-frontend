    ...
    <div style={{ animation: "slideIn 0.3s ease" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "12px" }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: "13px", color: "#9ca3af" }}>{total.toLocaleString()} total events</p>
        </div>
...
export default function Admin() {
  ...
  // ─── FETCH ALL DATA ────────────────────────────────────────────────────────
  // FIX: Poll every 60s instead of 15s.
  // 15s × 5 endpoints = 300 req/15min → exceeded rate limit → 503
  // 60s × 5 endpoints = 75 req/15min → well under any limit
  // FIND and REPLACE the entire fetchAll useEffect block:
useEffect(() => {
  ...
  const addMedicine = async () => {
    ...
    try {
      ...
      setImgPreview("");
      axios.get(`${BASE_URL}/medicines/all`, { withCredentials: true, headers: getAuthHeaders() }).then((r) => setMedicines(sanitizeObjectArray(r.data))).catch(() => {});
    } catch (err) {
  ...
  const saveEditMedicine = async () => {
    try {
      await axios.put(`${BASE_URL}/medicines/${editingMed._id}`, {
        ...
        supplier: editingMed.supplier || "", expiryDate: editingMed.expiryDate || "", entryDate: editingMed.entryDate || "",
      }, { withCredentials: true, headers: getAuthHeaders() });
  ...
  const deleteMedicine = async (id) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/medicines/${id}`, { withCredentials: true, headers: getAuthHeaders() });
  ...
  const permanentDeleteMedicine = async (id, name) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/medicines/${id}/permanent`, { withCredentials: true, headers: getAuthHeaders() });
  ...
  const saveStockUpdate = async () => {
    ...
    try {
      ...
      axios.get(`${BASE_URL}/medicines/low-stock`, { withCredentials: true, headers: getAuthHeaders() }).then((r) => setLowStockMeds(sanitizeObjectArray(r.data))).catch(() => {});
    } catch {
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      await axios.patch(`${BASE_URL}/orders/${orderId}/status`, { status: newStatus },
        { withCredentials: true, headers: getAuthHeaders() });
  ...
  const createUser = async () => {
    ...
    try {
      await axios.post(`${BASE_URL}/users/create`, userForm, { withCredentials: true, headers: getAuthHeaders() });
      ...
      fetch(`${BASE_URL}/users`, { credentials: "include", headers: getAuthHeaders() }).then((r) => r.ok ? r.json() : []).then((p) => setUsers(sanitizeObjectArray(p))).catch(() => {});
    } catch (err) {
  ...
  const updateUser = async () => {
    ...
    try {
      ...
      await axios.patch(`${BASE_URL}/users/${editingUser._id}`, payload, { withCredentials: true, headers: getAuthHeaders() });
  ...
  const deleteUser = async (userId, userName) => {
    ...
    try {
      await axios.delete(`${BASE_URL}/users/${userId}`, { withCredentials: true, headers: getAuthHeaders() });
  ...
  const toggleDisableUser = async (userId, isCurrentlyDisabled, userName) => {
    try {
      await axios.patch(`${BASE_URL}/users/${userId}/disable`, {}, { withCredentials: true, headers: getAuthHeaders() });
  ...
  const clearNotice = async () => {
    try {
      await fetch(`${BASE_URL}/notice`, { method: "DELETE", credentials: "include", headers: getAuthHeaders() });
  ...
  const searchUserByPhone = async (phone) => {
    ...
    try {
      const res = await axios.get(`${BASE_URL}/users/search?phone=${phone}`, { withCredentials: true, headers: getAuthHeaders() });
  ...
  const posPlaceOrder = async () => {
    ...
    try {
      ...
      axios.get(`${BASE_URL}/medicines/all`, { withCredentials: true, headers: getAuthHeaders() }).then((r) => setMedicines(sanitizeObjectArray(r.data))).catch(() => {});
      axios.get(`${BASE_URL}/orders`, { withCredentials: true, headers: getAuthHeaders() }).then((r) => setOrders(sanitizeObjectArray(r.data))).catch(() => {});
    } catch (err) {
  ...
  const fetchActivityLogs = async (pg = 1, action = "") => {
    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/activity-logs?${params}`, { credentials: "include", headers: getAuthHeaders() });
      if (res.ok) {
  ...
  const handleLogout = async () => {
    ...
    ["isLoggedIn","role","email","name","phone","userId","token"].forEach(k => localStorage.removeItem(k));
    ...
    <div style={{ height: "100vh", overflow: "hidden", background: "#f1f5f9", fontFamily: "'Plus Jakarta Sans','Segoe UI', system-ui, sans-serif", display: "flex" }}>
      ...
      <aside className="sidebar-desktop" style={{
          ...
          {tabs.map(tab => {
            ...
            return (
              <button key={tab.id} className="sb-nav-item" onClick={() => setActiveTab(tab.id)} style={{
                ...
                fontWeight: isActive ? "700" : "400", fontSize:"13px", position:"relative",
              }}>
        ...
        <div style={{padding:"12px 8px",borderTop:"1px solid rgba(255,255,255,.09)"}}>
          <button className="sb-nav-item" onClick={() => navigate("/store")} style={{display:"flex",alignItems:"center",gap:"10px",padding:"10px 13px",borderRadius:"10px",background:"rgba(34,197,94,.15)",color:"#86efac",fontSize:"13px",marginBottom:"4px"}}>
            <span style={{fontSize:"15px",width:"18px",textAlign:"center"}}>🛒</span><span>Visit Store</span>
          </button>
          <button className="sb-nav-item" onClick={handleLogout} style={{display:"flex",alignItems:"center",gap:"10px",padding:"10px 13px",borderRadius:"10px",background:"rgba(239,68,68,.15)",color:"#fca5a5",fontSize:"13px"}}>
            <span style={{fontSize:"15px",width:"18px",textAlign:"center"}}>🚪</span><span>Logout</span>
          </button>
        </div>
      </aside>
      ...
      {sidebarOpen && (
        <div className="mob-overlay" style={{position:"fixed",inset:0,zIndex:200,display:"flex"}}>
          <aside style={{width:"220px",height:"100%",overflowY:"auto",background:"linear-gradient(180deg,#071810 0%,#0d3320 45%,#166534 100%)",display:"flex",flexDirection:"column",animation:"sideIn .25s ease",boxShadow:"4px 0 24px rgba(0,0,0,.3)"}}>
            ...
            <nav style={{flex:1,padding:"10px 8px"}}>
              {tabs.map(tab => (
                <button key={tab.id} className="sb-nav-item" onClick={() => { setActiveTab(tab.id); setSidebarOpen(false); }} style={{display:"flex",alignItems:"center",gap:"10px",padding:"11px 13px",borderRadius:"10px",marginBottom:"2px",background:activeTab===tab.id?"rgba(255,255,255,.18)":"transparent",color:activeTab===tab.id?"white":"rgba(255,255,255,.6)",fontWeight:activeTab===tab.id?"700":"400",fontSize:"13px"}}>
            ...
            <div style={{padding:"12px 8px",borderTop:"1px solid rgba(255,255,255,.1)"}}>
              <button className="sb-nav-item" onClick={() => { navigate("/store"); setSidebarOpen(false); }} style={{display:"flex",alignItems:"center",gap:"10px",padding:"11px 13px",borderRadius:"10px",background:"rgba(34,197,94,.15)",color:"#86efac",fontSize:"13px",marginBottom:"4px"}}><span>🛒</span><span>Visit Store</span></button>
              <button className="sb-nav-item" onClick={handleLogout} style={{display:"flex",alignItems:"center",gap:"10px",padding:"11px 13px",borderRadius:"10px",background:"rgba(239,68,68,.15)",color:"#fca5a5",fontSize:"13px"}}><span>🚪</span><span>Logout</span></button>
            </div>
          </aside>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,overflowY:"auto",marginLeft:"220px"}} className="admin-main-area">
                  ...
                  return (
                    <div key={status} style={{ ...styles.statusBox, background: sc.bg, cursor: "pointer" }} onClick={() => setActiveTab("orders")}>
                      ...
                      <div style={{ height: "5px", background: "rgba(0,0,0,0.08)", borderRadius: "3px" }}>
                        <div style={{ width: `${pct}%`, height: "100%", background: sc.color, borderRadius: "3px" }} />
                      </div>
                    </div>
                  );
                })}
                    ...
                    {recentOrders.map((order) => {
                      ...
                      return (
                        <TableRow key={order._id} hover>
                          ...
                          <TableCell>
                            <span style={{ fontWeight: "600", fontSize: "13px" }}>{order.orderType === "walk-in" ? (order.guestInfo?.name || "Walk-in") : (order.userId?.name || "Unknown")}</span>
        ...
        {activeTab === "analytics" && (
          <div style={{ animation: "slideIn 0.3s ease" }}>
            {analyticsLoading ? (
              <div style={{background:"white",borderRadius:"14px",padding:"80px",textAlign:"center"}}>
                <div style={{width:"40px",height:"40px",border:"4px solid #dcfce7",borderTop:"4px solid #166534",borderRadius:"50%",animation:"spin 0.8s linear infinite",margin:"0 auto 16px"}}/>
              ...
              <div style={{background:"white",borderRadius:"16px",padding:"80px 32px",textAlign:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
                ...
                <p style={{color:"#888",fontSize:"14px",maxWidth:"380px",margin:"0 auto 28px",lineHeight:1.7}}>Analytics appear once orders are placed.</p>
                <button onClick={fetchAnalytics} style={{padding:"11px 28px",background:"#166534",color:"white",border:"none",borderRadius:"10px",fontWeight:"600",cursor:"pointer",fontSize:"14px",marginRight:"12px"}}>↻ Refresh</button>
                <button onClick={() => setActiveTab("pos")} style={{padding:"11px 28px",background:"#f0fdf4",color:"#166534",border:"1px solid #bbf7d0",borderRadius:"10px",fontWeight:"600",cursor:"pointer",fontSize:"14px"}}>Create Walk-in Order →</button>
              </div>
            ) : (
              <div>
                ...
                <div style={{background:"white",borderRadius:"14px",padding:"28px",boxShadow:"0 2px 8px rgba(0,0,0,0.07)"}}>
                  <h3 style={{margin:"0 0 20px",fontSize:"17px",fontWeight:"700",color:"#111"}}>Best selling medicines</h3>
                  {analytics.topMedicines.length === 0 ? (
                    <div style={{textAlign:"center",padding:"40px",color:"#9ca3af"}}>No sales data yet</div>
                  ) : analytics.topMedicines.map((med,i)=>(
                    <div key={med.name} className="row-hover" style={{display:"flex",alignItems:"center",gap:"12px",padding:"12px",borderRadius:"8px"}}>
                      <div style={{width:"28px",height:"28px",borderRadius:"50%",background:i<3?"#fef3c7":"#f3f4f6",display:"flex",alignItems:"center",justifyContent:"center",fontSize:i<3?"14px":"12px",fontWeight:"700",color:"#374151",flexShrink:0}}>{i===0?"🥇":i===1?"🥈":i===2?"🥉":i+1}</div>
                      <div style={{flex:1,fontSize:"14px",fontWeight:i===0?"700":"500"}}>{med.name}</div>
                      <div style={{background:"#f0fdf4",color:"#166534",padding:"3px 10px",borderRadius:"20px",fontSize:"13px",fontWeight:"700"}}>{med.totalQty} units</div>
                      <div style={{fontSize:"14px",fontWeight:"600",color:"#166534",minWidth:"80px",textAlign:"right"}}>Rs.{med.totalRevenue.toLocaleString()}</div>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  <div key={type} style={{background:"white",borderRadius:"16px",boxShadow:"0 2px 8px rgba(0,0,0,0.07)",overflow:"hidden"}}>
                    ...
                    {serving>0&&(
                      <div style={{margin:"0 20px 20px",background:bg,border:`1px solid ${border}`,borderRadius:"10px",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                        ...
                        <span style={{fontSize:"20px",fontWeight:"700",color,letterSpacing:"0.05em"}}>{type==="appointment"?"APT":type==="walkin"?"WLK":"ORD"}-{String(serving).padStart(3,"0")}</span>
                      </div>
                    )}
                  </div>
                );
          ...
          <div style={{ ...styles.card, animation: "slideIn 0.3s ease" }}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"20px",flexWrap:"wrap",gap:"12px"}}>
              <div>
                ...
                <p style={{margin:"4px 0 0",fontSize:"13px",color:"#9ca3af"}}>{filteredOrders.length} orders total</p>
              </div>
          ...
          <div style={{ ...styles.card, animation: "slideIn 0.3s ease" }}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"20px",flexWrap:"wrap",gap:"12px"}}>
              <div>
                ...
                <p style={{color:"#9ca3af",fontSize:"13px",margin:"4px 0 0"}}>{safeAppointments.length} total · {safeAppointments.filter(a=>a.status==="Pending").length} pending</p>
              </div>
                    ...
                    {filteredApts.map((apt, idx) => {
                      ...
                      return (
                        <TableRow key={apt._id || idx} hover>
                          <TableCell>
                            <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                              <div style={{width:"32px",height:"32px",borderRadius:"50%",background:"linear-gradient(135deg,#166534,#4ade80)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontWeight:"700",fontSize:"13px",flexShrink:0}}>{(apt.name||"?").charAt(0).toUpperCase()}</div>
              ...
              <div style={{...styles.card,border:"2px solid #166534"}}>
                ...
                {posCart.length===0?(
                  ...
                  <div>
                    ...
                    <button onClick={posPlaceOrder} disabled={posPlacing} style={{width:"100%",padding:"14px",background:posPlacing?"#9ca3af":"#166534",color:"white",border:"none",borderRadius:"10px",fontWeight:"700",fontSize:"15px",cursor:posPlacing?"not-allowed":"pointer"}}>
        ...
        {activeTab === "inventory" && (
          <div style={{ animation: "slideIn 0.3s ease" }}>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:"14px",marginBottom:"24px"}}>
              {[{label:"Total Medicines",value:safeMedicines.length,icon:"💊",color:"#166534"},{label:"Active in Store",value:safeMedicines.filter(m=>m.isActive).length,icon:"✅",color:"#1e40af"},{label:"Low Stock",value:lowStockMeds.filter(m=>m.stock>0).length,icon:"⚠️",color:"#92400e"},{label:"Out of Stock",value:lowStockMeds.filter(m=>m.stock<=0).length,icon:"🚫",color:"#991b1b"},{label:"Hidden",value:safeMedicines.filter(m=>!m.isActive).length,icon:"👁️",color:"#6b7280"}].map(({label,value,icon,color})=>(
            ...
            <div style={styles.card}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"16px",marginTop:"16px"}}>
                ...
                <div><label style={styles.fieldLabel}>Unit</label><select value={medForm.unit} onChange={(e)=>setMedForm({...medForm,unit:e.target.value})} style={{...styles.inputField,background:"white"}}>{["units","bottles","strips","boxes","sachets","vials"].map((u)=><option key={u} value={u}>{u.charAt(0).toUpperCase()+u.slice(1)}</option>)}</select></div>
                <div><label style={styles.fieldLabel}>Supplier</label><input placeholder="e.g. Sun Pharma" value={medForm.supplier} onChange={(e)=>setMedForm({...medForm,supplier:e.target.value})} style={styles.inputField}/></div>
                ...
                <div><label style={styles.fieldLabel}>Entry Date</label><input type="date" value={medForm.entryDate} onChange={(e)=>setMedForm({...medForm,entryDate:e.target.value})} style={styles.inputField}/></div>
                ...
                <div><label style={styles.fieldLabel}>Image</label><label style={styles.fileLabel}>📷 Upload<input type="file" accept="image/*" onChange={handleImageSelect} style={{display:"none"}}/></label>{imgPreview&&<img src={imgPreview} alt="preview" style={{width:"48px",height:"48px",borderRadius:"7px",objectFit:"cover",marginLeft:"10px",verticalAlign:"middle"}}/>}</div>
              </div>
            ...
            <div style={styles.card}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px"}}>
                ...
                <button onClick={()=>window.open("/store","_blank")} style={{display:"flex",alignItems:"center",gap:"7px",padding:"8px 16px",background:"linear-gradient(135deg,#166534,#15803d)",color:"white",border:"none",borderRadius:"9px",fontSize:"13px",fontWeight:"700",cursor:"pointer"}}>🛒 Preview in Store</button>
              </div>
              <TableContainer component={Paper} elevation={0} style={{border:"1px solid #e5e7eb",borderRadius:"10px",marginTop:"16px"}}>
                <Table>
                  <TableHead style={{background:"#f9fafb"}}>
                    <TableRow>{["Medicine","Category","Price","Stock","Supplier","Expiry","Status","Visibility","Actions"].map((h)=>(<TableCell key={h}><strong style={{fontSize:"12px",color:"#374151"}}>{h}</strong></TableCell>))}</TableRow>
                  </TableHead>
                  <TableBody>
                    {safeMedicines.map((med)=>{
                      ...
                      return (
                        <TableRow key={med._id} hover style={{opacity:med.isActive?1:0.5}}>
                          <TableCell><div style={{display:"flex",alignItems:"center",gap:"10px"}}>{med.img&&<img src={med.img} alt={med.name} style={{width:"38px",height:"38px",borderRadius:"8px",objectFit:"cover",flexShrink:0}}/>}<div><div style={{fontWeight:"600",fontSize:"13px"}}>{med.name}</div><div style={{fontSize:"11px",color:"#9ca3af"}}>{med.desc?.slice(0,38)||""}</div></div></div></TableCell>
                          ...
                          <TableCell>{(()=>{const expSt=getExpiryStatus(med.expiryDate);if(!expSt)return<span style={{color:"#d1d5db",fontSize:"12px"}}>—</span>;return<span style={{display:"inline-block",background:expSt.bg,color:expSt.color,padding:"2px 9px",borderRadius:"8px",fontSize:"11px",fontWeight:"600"}}>{expSt.days<0?"⛔ Expired":expSt.days<=90?`⚠️ ${expSt.label}`:`✓ ${new Date(med.expiryDate).toLocaleDateString("en-IN",{day:"numeric",month:"short",year:"2-digit"})}`}</span>;})()}</TableCell>
                          ...
                          <TableCell>
                            ...
                            <button onClick={()=>axios.put(`${BASE_URL}/medicines/${med._id}`,{isActive:!med.isActive},{withCredentials:true,headers:getAuthHeaders()}).then(()=>setMedicines((prev)=>prev.map((m)=>m._id===med._id?{...m,isActive:!med.isActive}:m)))} style={{padding:"4px 10px",background:med.isActive?"#fee2e2":"#dcfce7",color:med.isActive?"#991b1b":"#166534",border:"none",borderRadius:"6px",cursor:"pointer",fontWeight:"600",fontSize:"12px",margin:"0 4px"}}>{med.isActive?"Hide":"Show"}</button>
                            <Tooltip title="Permanently delete from database"><button onClick={()=>permanentDeleteMedicine(med._id,med.name)} style={{padding:"4px 10px",background:"#fee2e2",color:"#991b1b",border:"1px solid #fecaca",borderRadius:"6px",cursor:"pointer",fontWeight:"600",fontSize:"12px"}}>🗑️ Delete</button></Tooltip>
                          </TableCell>
                        </TableRow>
                      );
                    })}
                      ...
                      <TableRow key={idx} hover style={{opacity:user.isDisabled?0.5:1}}>
                        ...
                        <TableCell>
                          <div style={{display:"flex",gap:"4px",flexWrap:"wrap"}}>
                            ...
                            <Tooltip title={user.isDisabled?"Enable account":"Disable account"}><button onClick={()=>toggleDisableUser(user._id,user.isDisabled,user.name)} style={{padding:"3px 8px",background:user.isDisabled?"#dcfce7":"#fef3c7",color:user.isDisabled?"#166534":"#92400e",border:"none",borderRadius:"6px",cursor:"pointer",fontSize:"11px",fontWeight:"600"}}>{user.isDisabled?"Enable":"Disable"}</button></Tooltip>
                            <Tooltip title="Delete"><button onClick={()=>deleteUser(user._id,user.name)} style={{padding:"3px 8px",background:"#fee2e2",color:"#991b1b",border:"1px solid #fecaca",borderRadius:"6px",cursor:"pointer",fontSize:"11px",fontWeight:"600"}}>🗑️</button></Tooltip>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))}
        ...
        {activeTab === "activity" && (
          <ActivityLogTab
            logs={activityLogs} total={activityTotal} page={activityPage}
            loading={activityLoading} filter={activityFilter} setFilter={setActivityFilter}
      ...
      {/* ✅ FIX 2: selectedApt._id (was selectedApt.id — undefined in MongoDB) */}
        ...
        <DialogContent>
          {editingMed && (
            <Box sx={{ pt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              <TextField label="Supplier" value={editingMed.supplier||""} onChange={(e)=>setEditingMed({...editingMed,supplier:e.target.value})} fullWidth size="small"/>
      ...
      <Dialog open={stockUpdateOpen} onClose={() => setStockUpdateOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <FormControl fullWidth size="small"><InputLabel>Operation</InputLabel><Select value={stockOperation} onChange={(e)=>setStockOperation(e.target.value)} label="Operation"><MenuItem value="add">➕ Add to stock</MenuItem><MenuItem value="subtract">➖ Remove from stock</MenuItem><MenuItem value="set">📝 Set exact value</MenuItem></Select></FormControl>
            <TextField label={stockOperation==="add"?"Quantity to Add":stockOperation==="subtract"?"Quantity to Remove":"Set Stock To"} type="number" value={stockValue} onChange={(e)=>setStockValue(e.target.value)} fullWidth size="small"/>
            {stockValue&&!isNaN(stockValue)&&(
              <Typography variant="body2" style={{color:"#166534",background:"#f0fdf4",padding:"10px",borderRadius:"8px"}}>
                New stock will be: <strong>{stockOperation==="add"?Number(stockMed?.stock||0)+Number(stockValue):stockOperation==="subtract"?Math.max(0,Number(stockMed?.stock||0)-Number(stockValue)):Number(stockValue)} {stockMed?.unit||"units"}</strong>
              </Typography>
            )}
          </Box>
        </DialogContent>
      ...
      <Dialog open={userFormOpen} onClose={() => setUserFormOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ pt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <FormControl fullWidth size="small"><InputLabel>Role</InputLabel><Select value={userForm.role} onChange={(e)=>setUserForm((f)=>({...f,role:e.target.value}))} label="Role"><MenuItem value="admin">🔑 Admin</MenuItem><MenuItem value="staff">🏥 Staff</MenuItem><MenuItem value="reception">🖥️ Reception</MenuItem><MenuItem value="user">👤 Patient</MenuItem></Select></FormControl>
          </Box>
        </DialogContent>
        <DialogActions style={{padding:"16px"}}>
          <Button onClick={()=>setUserFormOpen(false)} style={{color:"#888"}}>Cancel</Button>
          <Button onClick={createUser} disabled={userFormLoading} variant="contained" style={{background:"#166534",color:"white"}}>{userFormLoading?"Creating...":"Create User"}</Button>
        </DialogActions>
      </Dialog>
        ...
        <DialogContent>
          {editingUser && (
            <Box sx={{ pt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              <FormControl fullWidth size="small"><InputLabel>Role</InputLabel><Select value={editingUser.role||"user"} onChange={(e)=>setEditingUser((u)=>({...u,role:e.target.value}))} label="Role"><MenuItem value="admin">🔑 Admin</MenuItem><MenuItem value="staff">🏥 Staff</MenuItem><MenuItem value="reception">🖥️ Reception</MenuItem><MenuItem value="user">👤 Patient</MenuItem></Select></FormControl>
            </Box>
          )}
        </DialogContent>
        <DialogActions style={{padding:"16px",display:"flex",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",gap:"8px"}}>
            ...
            <Button onClick={updateUser} disabled={userFormLoading} variant="contained" style={{background:"#166534",color:"white"}}>{userFormLoading?"Saving...":"Save Changes"}</Button>
          </div>
        </DialogActions>
      </Dialog>
